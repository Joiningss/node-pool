...
exports.Pool = function (factory) {
  ...
  /**
   * Change factory.max dynamic. by Joiningss
   *
   * @param {Number} max
   *  Maximum number of items that can exist at the same time.
   *
   *  TODO: if new max is litter then original max, call some method like ensureMax func to limit
   *  count of availableObjects + clients in work.
   */

  me.changeMax = function(max){
    factory.max = parseInt(max,10);
    var waitingCount = waitingClients.size();
    if(waitingCount>0 && !draining){
      dispense();
    }
  }
